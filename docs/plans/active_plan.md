...
- [x] **Sub-stage 5.9: `import_export` (Universal Streaming Bulk Processing)** - COMPLETED (Commit: `19f0597`)
...
- [x] **Sub-stage 5.10: `backup` (Unified Atomic Archive Engine)** - COMPLETED (Commit: `f5621ab`)
  - Atomic archive generator packaging `dump.sql` (PostgreSQL raw dump), `filestore/` (blob attachments), and `manifest.json` into compressed `.tar.gz` bundles (`BackupEngine`).
  - Cryptographic SHA-256 verification, backup tracking model (`BackupRecord`), and REST download/verify endpoints (`/api/v1/backup/*`).
- [x] **Sub-stage 5.11: `i18n` (Multi-Language & Localization)** - COMPLETED (Commit: `632cca9`)
  - Multi-lingual field JSONB structure (`{"en": "...", "ar": "..."}`), RTL detection, and `TranslationTerm` models.
  - Locale resolution middleware (`Accept-Language`), centralized translation catalog compilation, and REST endpoints (`/api/v1/i18n/*`).
- [x] **Sub-stage 5.12: Primary Root Admin Immunity & Hierarchical Superuser Governance (`identity_rbac`)** - COMPLETED (Commit: `63ce332`)
  - Data model: `is_primary_admin: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)` on `User`.
  - Setup CLI: `backend/setup_database.py` seeds initial Super Admin with `is_primary_admin = True`.
  - Route guards:
    - Primary Admin immunity against soft-deletion by anyone (`403 Forbidden`).
    - Primary Admin immunity against modification or deactivation by secondary superusers (`403 Forbidden`).
    - Self-demotion / self-deactivation protection for Primary Admin (`400 Bad Request`).
    - Secondary superusers blocked from creating other superusers (`403 Forbidden`).
...
- [x] **Sub-stage 5.14: Email-Verified Forgot/Reset Password Flow & Sign-up Verification (Mail Gateway Bridge)** - COMPLETED
  - Redis-backed time-limited (15-min TTL), single-use token service in `identity_rbac/security.py` using atomic `GETDEL`.
  - `POST /api/v1/identity_rbac/auth/forgot-password` (timing attack safe, dispatches single-use token via `MailService.enqueue_mail`).
  - `POST /api/v1/identity_rbac/auth/reset-password` (validates token, atomically burns token, updates hash, enforces complexity and match rules).
  - Added `email_verified` column to `User` and implemented `POST /api/v1/identity_rbac/auth/verify-email` endpoint.
  - Unit tests verifying forgot/reset password lifecycle, token single-use burning/replay protection, and registration email verification (17/17 tests passing).
- [x] **Sub-stage 5.15: Two-Factor Authentication (2FA / RFC 6238 TOTP Subsystem)** - COMPLETED
  ...
  - Unit tests verifying complete 2FA lifecycle and 2-step login challenge (18/18 tests passing).
- [x] **Sub-stage 5.16: Universal Modular Settings Engine Architecture & Module Refactor** - COMPLETED
  - Self-describing typed settings schemas per module (`settings.py`) with title/label, description, type, default, and options/choices.
  - `SettingsService` centralized registry, schema introspection endpoint, and transparent fallback default merging.
  - Decouple operational policies (`allow_registration`) from `Company` SQL table; migrate to `ModuleSettings["identity_rbac"]`.
  - Audit and define structured settings for `identity_rbac`, `mail_gateway`, `i18n`, `audit`, `backup`, and `notification_engine`.
  - Codify settings architectural rule in `docs/ai_wiki/architecture.md` and workspace guidelines for all AI agents.
  - Verify full test suite passing and re-export OpenAPI and Postman specs.
- [x] **Sub-stage 5.17: Universal Hierarchical Category Engine & CategorizableMixin** - COMPLETED
  - Defined canonical `Category` model (`lookup_categories`) supporting multi-tenancy, polymorphic scope (`res_model`), optional `parent_id` self-reference for sub-classifications, sequence ordering, color, and icon.
  - Universal `CategorizableMixin` in `core/base_models.py` equipping any domain model with `category_id` FK.
  - `CategoryService` implementing tree construction, breadcrumb path traversal, and strict cycle prevention (rejecting self or descendant parent loops with HTTP 400).
  - Extended `DocumentAttachment` and `MailTemplate` to inherit `CategorizableMixin`.
  - Added REST CRUD & tree endpoints (`/api/v1/lookups/categories` & `/api/v1/lookups/categories/tree`).
  - Seeded default hierarchical categories in `fixtures.py` and `setup_database.py`.
  - Comprehensive unit tests (68/68 passing in Docker container).
- [x] **Sub-stage 5.18: Lookups Master Data Full CRUD, PATCH Support & Hierarchical Postman Sub-Folders** - COMPLETED
  - Typed update schemas in `schemas.py`: `CountryUpdate`, `CityUpdate`, `CurrencyUpdate`, `UnitOfMeasureUpdate`, `TaxTypeUpdate`, `TagUpdate`.
  - Added REST endpoints `GET /{id}`, `PATCH /{id}`, and `DELETE /{id}` for Country, City, Currency, UnitOfMeasure, TaxType, Tag in `routes.py` with multi-tenant isolation and soft-delete.
  - Upgraded Postman Exporter (`backend/core/exporter.py`) with hierarchical sub-folder generation based on route tags, organizing Lookups into `Categories`, `Units of Measure`, `Countries`, `Cities`, `Currencies`, `Tax Types`, `Tags`, and root `Bootstrap Company ISO Lookups`.
  - Dynamic Postman `{id}` path parameter resolution to `{{active_<resource>_id}}` and automated ID capture on creation.
...
- [x] **Sub-stage 6.1: Trigger Registry & Lifecycle Interceptors** - COMPLETED (Commit: `312722f`)
...
- [x] **Sub-stage 6.2: Condition Expression Evaluator** - COMPLETED (Commit: `312722f`)
...
- [x] **Sub-stage 6.3: Built-in Action Handlers & Dynamic Registry** - COMPLETED (Commit: `312722f`)
...
- [x] **Sub-stage 6.4: Execution History & Resilience** - COMPLETED (Commit: `312722f`)
...
- *2026-09-14 (Sub-stage 5.9 Completed - Commit: `19f0597`)*: Implemented `import_export` module: universal `DataProcessor` parsing and generating CSV, Excel (.xlsx via `openpyxl`), and JSON, `ImportExportJob` lifecycle tracking with error logs and row metrics, asynchronous Celery execution (`process_import_export_job_task`), and REST endpoints for multipart upload, export triggering, status polling, and binary file download (`/api/v1/import_export/*`). Synchronized continuous OpenAPI and Postman collections. All 46 unit tests passing in container in 25.04s.
- *2026-09-14 (Sub-stage 5.10 Completed - Commit: `f5621ab`)*: Implemented `backup` module: `BackupEngine` extracting tenant-scoped SQL DDL/INSERT statements and archiving with filestore blobs into compressed `.tar.gz` bundles with `manifest.json`, cryptographic SHA-256 validation, `BackupRecord` tracking model, Celery asynchronous execution (`generate_backup_task`), and REST endpoints (`/api/v1/backup/*`). Synchronized continuous OpenAPI and Postman collections. All 49 unit tests passing in container in 27.39s.
- *2026-09-14 (Sub-stage 5.11 Completed - Commit: `632cca9`)*: Implemented `i18n` module (concluding Stage 5 in full): multi-lingual JSONB translatable fields, `I18nService.resolve_field` resolution, `MultiTenancyContextMiddleware` locale detection from `Accept-Language` / `X-Locale`, Arabic RTL first-class citizen support, `TranslationTerm` model, compiled catalog retrieval, dynamic string translation, and REST endpoints (`/api/v1/i18n/*`). Synchronized continuous OpenAPI and Postman collections. All 52 unit tests passing in container in 29.89s.
- *2026-09-14 (Architectural Enhancement: Tenant Registration Gating & Setup CLI - Commit: `a35b813`)*: Implemented `Company` tenant entity with `allow_registration` boolean toggle. Hardened `POST /api/v1/identity_rbac/auth/register` to reject registration with `403 Forbidden` if `allow_registration` is False or `404 Not Found` if company does not exist. Added internal user provisioning endpoint (`POST /api/v1/identity_rbac/users`) and company administration endpoints (`/api/v1/identity_rbac/companies`). Created interactive database installer CLI (`backend/setup_database.py`) with strict username and password validation, PostgreSQL schema wipe/recreate, ISO lookups seeding, Super Admin provisioning with universal access, and automated Postman collection/environment synchronization. Verified 56/56 tests passing.
- *2026-09-14 (Identity RBAC Full CRUD, Soft-Deletes, Permission Taxonomy & Deep Postman Docs - Commit: `1d0728f`)*: Upgraded `identity_rbac` to complete enterprise CRUD: added `DELETE` endpoints for Company, User, Group, and Permission with `SoftDeleteMixin` (`deleted_at = utcnow()`, `deleted_by_id = current_user.id`) and self-deletion safety guard. Implemented `PATCH` endpoints for User, Group, and Permission with typed Pydantic update schemas (`UserUpdate`, `GroupUpdate`, `PermissionUpdate`). Formalized granular permission attributes: `module_name`, `resource`, `action` (`read`, `create`, `update`, `delete`, `manage`, `approve`, `export`), `ownership_scope` (`GLOBAL`, `TEAM`, `OWN`), and canonical code derivation `{module_name}.{resource}.{action}`. Upgraded `backend/core/exporter.py` with `_format_markdown_docs`: automatically generates and injects rich GitHub-flavored Markdown tables into Postman request descriptions detailing field names, types, required flags, allowed enum choices, and descriptions, directly priming the schema surface for Stage 7 FastMCP reflection by Hermes. Formalized Section 6.3 in `docs/ai_wiki/architecture.md` ("Modular Deactivation & Polymorphic Linkage Standard"). Verified 60/60 tests passing across all 11 modules in Docker.
...
- *2026-09-14 (Sub-stage 5.13 Completed - Commit: `d7416c3`)*: Decoupled credentials from general profile patching: completely removed `password` field from `UserUpdate` schema and `PATCH /api/v1/identity_rbac/users/{user_id}` route. Implemented dedicated authenticated password change endpoint `POST /api/v1/identity_rbac/auth/change-password` with `current_password` verification against bcrypt hash, `new_password` vs `confirm_password` match validation, and prevention of identical current-to-new password churn. Added `test_authenticated_change_password_and_decoupling` with 7 validation vectors. Regenerated OpenAPI and Postman collections. All 64/64 tests passing in container.
- *2026-09-14 (Sub-stage 5.14 Completed - Commit: `130d0cd`)*: Implemented email-verified password reset lifecycle and registration email verification: added single-use Redis-backed token lifecycle in `security.py` using atomic `GETDEL` for replay protection, added `POST /api/v1/identity_rbac/auth/forgot-password` with constant response time to defeat email enumeration, added `POST /api/v1/identity_rbac/auth/reset-password` validating and consuming tokens, added `email_verified` column on `User` table, and added `POST /api/v1/identity_rbac/auth/verify-email`. Integrated automated welcome verification email dispatch via `MailService.enqueue_mail` on self-registration. Synchronized OpenAPI and Postman collection. All 66/66 tests passing in container.
- *2026-09-14 (Sub-stage 5.15 Completed - Commit: `6004126`)*: Implemented complete enterprise Two-Factor Authentication (2FA / RFC 6238 TOTP Subsystem): added `pyotp` dependency, extended `User` table with `two_factor_enabled`, `two_factor_secret`, and `two_factor_recovery_codes` JSONB. Implemented `POST /api/v1/identity_rbac/auth/2fa/setup` generating Base32 secret and QR provisioning URI, `POST /api/v1/identity_rbac/auth/2fa/enable` activating 2FA and generating 8 emergency recovery codes stored as cryptographic SHA-256 hashes, and `POST /api/v1/identity_rbac/auth/2fa/disable` requiring current password re-verification and valid TOTP or recovery code. Updated `POST /api/v1/identity_rbac/auth/login` to return an MFA challenge with `mfa_token`, completed via `POST /api/v1/identity_rbac/auth/2fa/verify` supporting both live TOTP codes and single-use emergency recovery code burning. Full test suite passing 67/67 tests in Docker container. Continuous OpenAPI and Postman collections synchronized.
- *2026-09-14 (Postman Request Titles & Documentation Refinement)*: Refined route `summary` parameters across all authentication, profile, password management, and 2FA endpoints in `backend/modules/base/identity_rbac/routes.py` to ensure unambiguous, developer-friendly titles in `docs/api/postman_collection.json` and `docs/api/openapi.json` (e.g. `Register New Account`, `Login (Username & Password)`, `Get Current User Profile`, `Change Password (In-App Settings)`, `Forgot Password (Step 1: Request Reset Email)`, `Forgot Password (Step 2: Confirm via Token)`, `Verify Email (Registration Confirmation)`, and prefixed `2FA: ...` workflows). Full test suite passing 67/67 tests in Docker container.
...
### 4. Current Focus
Stage 6.3 completed. All 93 automated tests passing in Docker container. Transitioning to Phase 43 per ratified enterprise architectural roadmap.

---

## Phase 43: Composable Enterprise Foundation & Transactional Core (The Enterprise Operating System)

- **Status**: IN_PROGRESS <!-- PENDING | IN_PROGRESS | COMPLETED -->
- **Active Branch**: `main`
- **Last Updated**: 2026-09-16 10:28:00+03:00
- **Architectural Reference**: [`docs/plans/backend_architecture_plan.md`](file:///home/ehab/Desktop/economy_editor/docs/plans/backend_architecture_plan.md) | Composable Domain Kernel

### 1. Objective & Scope
Transform the platform from a headless CRUD/data engine into a complete **Enterprise Operating System**. By implementing these foundational capabilities as decoupled **Base Utilities** (`backend/modules/base/`), all future business domain suites (CRM, Invoicing, Procurement, HR, Accounting, Inventory) become thin composition packages (10–20% code) that orchestrate these base models rather than re-inventing them.

#### **Already Completed Modules (Excluded from New Scope / Fully Operational)**
The following 13 base modules are already complete, tested, and active in the system:
1. `identity_rbac`: RBAC, 3-tier ownership (`GLOBAL`, `TEAM`, `OWN`), FLAC, 2FA TOTP, Root Admin immunity.
2. `settings`: Per-module dynamic typed JSONB settings with Redis caching and invalidation.
3. `lookups`: Normalized master data (`Country`, `City`, `Currency`, `UnitOfMeasure`, `TaxType`, `Tag`, `Category`).
4. `audit`: Immutable `AuditLog` table with automated state diff computation (`compute_instance_diff`).
5. `chatter`: Polymorphic threaded discussions `(res_model, res_id)`, activity reminders, and WebSocket gateway.
6. `documents`: Content-Addressable Storage (CAS with SHA-256) and parent-inherited permissions.
7. `mail_gateway`: Outbound SMTP, Jinja2 multi-lingual templates, and async Celery delivery.
8. `notification_engine`: Multi-channel push dispatcher (In-App WebSocket, WebPush VAPID, Mobile Push).
9. `import_export`: Universal streaming bulk processor for CSV, Excel (.xlsx), and JSON.
10. `backup`: Atomic disaster recovery bundles (`dump.sql` + `filestore/` + `manifest.json`).
11. `i18n`: Dynamic multi-lingual JSONB fields, RTL detection, and translation catalogs.
12. `automated_actions`: Event-driven Trigger-Condition-Action (TCA) engine with Celery background worker.
13. `reporting`: Universal headless dynamic report query compiler and ReportLab executive PDF renderer.
...
- [x] **Sub-stage 43.3.3: Tax Engine & Fiscal Positions (`taxes`)** - COMPLETED (Commit: `4dfe63d`)
...
- *2026-09-16*: Base Modules prioritized first (Stages 43.1 - 43.4), followed by the AI Bridge at the very end (Stage 43.5) per user explicit direction.
...
- **Status**: COMPLETED <!-- PENDING | IN_PROGRESS | COMPLETED -->
- **Active Branch**: `main`
- **Last Updated**: 2026-09-17 03:30:00+03:00
...
### 4. Current Focus
Phase 52 completed. Resuming Phase 51 (Frontend Platform): Connect `MasterShellWireframe.tsx` to dynamically consume `GET /api/v1/ui/menus` and render live top-launcher and sub-navigation menus.
