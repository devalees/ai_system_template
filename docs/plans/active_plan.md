...
- *2026-09-14 (Identity RBAC Full CRUD, Soft-Deletes, Permission Taxonomy & Deep Postman Docs)*: Upgraded `identity_rbac` to complete enterprise CRUD: added `DELETE` endpoints for Company, User, Group, and Permission with `SoftDeleteMixin` (`deleted_at = utcnow()`, `deleted_by_id = current_user.id`) and self-deletion safety guard. Implemented `PATCH` endpoints for User, Group, and Permission with typed Pydantic update schemas (`UserUpdate`, `GroupUpdate`, `PermissionUpdate`). Formalized granular permission attributes: `module_name`, `resource`, `action` (`read`, `create`, `update`, `delete`, `manage`, `approve`, `export`), `ownership_scope` (`GLOBAL`, `TEAM`, `OWN`), and canonical code derivation `{module_name}.{resource}.{action}`. Upgraded `backend/core/exporter.py` with `_format_markdown_docs`: automatically generates and injects rich GitHub-flavored Markdown tables into Postman request descriptions detailing field names, types, required flags, allowed enum choices, and descriptions, directly priming the schema surface for Stage 7 FastMCP reflection by Hermes. Formalized Section 6.3 in `docs/ai_wiki/architecture.md` ("Modular Deactivation & Polymorphic Linkage Standard"). Verified 60/60 tests passing across all 11 modules in Docker.
